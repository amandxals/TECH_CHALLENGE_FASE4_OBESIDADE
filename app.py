...
def load_artifacts():
    ...
    le_map      = pickle.load(open(f"{MODEL_DIR}/le_map.pkl",      "rb"))
    target_le   = pickle.load(open(f"{MODEL_DIR}/target_le.pkl",   "rb"))
    feature_cols= pickle.load(open(f"{MODEL_DIR}/feature_cols.pkl","rb"))
    metrics     = json.load(  open(f"{MODEL_DIR}/metrics.json",    "r"))
    return model, le_map, target_le, feature_cols, metrics

@st.cache_data
def load_data():
    df = pd.read_csv(f"{MODEL_DIR}/obesity_processed.csv")
    return df
